...
import 'package:home/screen/lpg/lpg.pagw3.dart';
...
class LPGPage2screen extends ConsumerStatefulWidget {
  final String billerCode;
  final String billerName;
  final String circleId;
  const LPGPage2screen({
    super.key,
  ...
  @override
  ConsumerState<LPGPage2screen> createState() => _LPGPage2screenState();
}

class _LPGPage2screenState extends ConsumerState<LPGPage2screen> {
...
class StateListDropdown extends ConsumerStatefulWidget {
  final Function(LpgStatesList) callBack;
...
class _StateListDropdownState extends ConsumerState<StateListDropdown> {
  ...
  Widget build(BuildContext context) {
    ...
    final stateListProvider = ref.watch(stateProvider);

    return stateListProvider.when(
      data: (snap) {
        // apply search filter
        final filteredList =
            snap.lpgStatesList.where((e) {
...
class DistrctListDropDown extends ConsumerStatefulWidget {
  ...
  final Function(LpgDistrictsList) callBack;
...
class _DistrctListDropDownState extends ConsumerState<DistrctListDropDown> {
  LpgDistrictsList? selectedCircle;
  String searchText = "";
  bool isDropdownOpen = false;
  ...
  Widget build(BuildContext context) {
    ...
    final stateListProvider = ref.watch(
      getDistrictListProvider(widget.stateId),
    );
...
class DistibruterLisDropDown extends ConsumerStatefulWidget {
  ...
  final Function(LpgDistributorsList) callBack;
...
class _DistibruterLisDropDownState
  ...
  LpgDistributorsList? selectedCircle;
  String searchText = "";
  bool isDropdownOpen = false;
  ...
  void initState() {
    // TODO: implement initState
    super.initState();
    bofy = {"stateID": widget.stateId, "districtId": widget.districId};
  }

  @override
  Widget build(BuildContext context) {
    final width = MediaQuery.of(context).size.width;
    final stateListProvider = ref.watch(getDistrubterProvider(bofy));

    return stateListProvider.when(
      data: (snap) {
        // apply search filter
        final filteredList =
            snap.lpgDistributorsList.where((e) {
              return e.distributorName.toLowerCase().contains(
                searchText.toLowerCase(),
              );
            }).toList();
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Padding(
                          padding: const EdgeInsets.symmetric(horizontal: 8.0),
                          child: Text(
                            selectedCircle?.distributorName ??
                                "Select Distributor",
                            overflow: TextOverflow.ellipsis,
                          ),
                        ),
