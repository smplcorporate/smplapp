...
class FastagSummeryPage extends ConsumerStatefulWidget {
  final FetchBodymodel body;
  FastagSummeryPage({required this.body});

  @override
  ConsumerState<FastagSummeryPage> createState() => _FastagSummeryPageState();
}

class _FastagSummeryPageState extends ConsumerState<FastagSummeryPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      final billData = ref.read(fetchBillDataProvider(fetchRequest));
      billData.whenData((e) {
        log("Fetched amount: ${e.billAmount}");
        _amountController.text = e.billAmount ?? "";
      });
    });
  ...
  Widget build(BuildContext context) {
    ...
    fetchBillerData.whenData(
      (e) => _amountController.text = e.billAmount ?? "",
    );
                                    ...
                                    onPressed: () async {
                                      if(applyBtnLoder == false){
                                        ...
                                        Fluttertoast.showToast(
                                          msg: "Billing amount required",
                                          backgroundColor: Colors.black,
                                          textColor: Colors.white,
                                        );
                                      } else {
                                        if (_coponKey.currentState!
                                            .validate()) {
                            ...
                            if (btnLoder == false) {
                              if (_amountController.text.trim().isEmpty) {
                                ...
                                if (_controller.text.isNotEmpty &&
                                  ...
                                  Fluttertoast.showToast(
                                    msg:
                                        "Please apply coupon code or enter amount",
                                    textColor: Colors.white,
                                    backgroundColor: Colors.red,
                                  );
                                  return;
